...
function isSuspiciousMinistry(ministry, headline = "") {
  const canonical = canonicalizePibSource(ministry);
  const m = normalizeMinistry(canonical).toLowerCase();
...
async function resolveMinistryForArticle(article) {
  ...
  const existingMinistry = canonicalizePibSource(article.ministry || "");
  ...
  if (
    ...
    return {
      ministry: canonicalizePibSource(summaryResult.ministry),
      source: "summary",
  ...
  if (ALLOW_FETCH && article.articleUrl) {
    ...
    if (
      ...
      return {
        ministry: canonicalizePibSource(fetchResult.ministry),
        source: "url_fetch",
